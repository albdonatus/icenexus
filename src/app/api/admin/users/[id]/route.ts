...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const user = await prisma.user.findUnique({
    where: { id },
    select: { id: true, name: true, email: true, role: true, active: true, companyId: true, phone: true },
  });
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { name, email, role, phone, companyId, active, password } = body;
  ...
  const data: Record<string, unknown> = { name, email, role, phone: phone || null, companyId: companyId || null, active };
